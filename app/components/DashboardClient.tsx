...
export default function DashboardClient({
  ...
  const symbols = useMemo(
    () => Array.from(new Set([...trendingCoins.map((tc) => tc.item.binanceSymbol), coin.binanceSymbol])),
  ...
  const columns = [
    {
      header: (
        <span className="bg-transparent border-none text-purple-100 font-medium whitespace-nowrap">Name</span>
      ),
    ...
    {
      header: (
        <span className="bg-transparent border-none text-purple-100 font-medium whitespace-nowrap">
          24h Change
        </span>
      ),
      cellClassName: 'name-cell border-dark-400 py-3 px-6 whitespace-nowrap',
      cell: (row: any) => {
        ...
        return (
          <div
            className={cn(
              'flex flex-col items-start gap-1 font-medium',
              isTrendingUp ? 'text-green-500' : 'text-red-500',
            )}
          >
            {isTrendingUp ? <TrendingUp width={18} height={18} /> : <TrendingDown width={18} height={18} />}
    ...
    {
      header: (
        <span className="bg-transparent border-none text-purple-100 font-medium whitespace-nowrap">Price</span>
      ),
  ...
  return (
    <section className="home-grid">
      {/* Chart column — slides in from left */}
      <motion.div
        id="coin-overview"
        className="w-full h-full xl:col-span-2 flex flex-col"
        ...
        >
          <div className="header pt-2">
            ...
            <div className="info">
              ...
              <h1>
                $
                {liveChartPrice.toLocaleString('en-US', {
                  minimumFractionDigits: liveChartPrice % 1 === 0 ? 0 : 2,
                  maximumFractionDigits: liveChartPrice % 1 === 0 ? 0 : 6,
                })}
              </h1>
            </div>
          </div>
      ...
      {/* Trending coins — slides in from right, slight delay */}
      <motion.div
        id="trending-coins"
        initial={{ opacity: 0, x: 28 }}
        animate={{ opacity: 1, x: 0 }}
